...
async function fetchEntitlementFromShopify(customerId) {
  if (!ACCESS_TOKEN) {
    ...
    return { tier: 'free', periodAnchor: null };
  }
  ...
  const activeAiPlusEdge = edges.find((edge) => {
  ...
  return { tier: 'ai_plus', periodAnchor: activeAiPlusEdge.node.createdAt };
}

/**
 * @returns {Promise<{ tier: 'free'|'ai_plus', periodAnchor: string|null }>}
 */
async function getEntitlement(customerId) {
  const key = cacheKey(customerId);
  const cached = await kv.get(key);
  if (cached && (cached.tier === 'free' || cached.tier === 'ai_plus')) {
    return cached;
  }

  let result;
  try {
    result = await fetchEntitlementFromShopify(customerId);
  } catch (err) {
