...
function createProjectsRepo(db = defaultDb) {
  ...
  // as a duplicate. Phase 6B: optional resolved `dates` ({ createdAt,
  // updatedAt } Date objects); database defaults apply when absent.
  async function importProject(customerId, fields, clientLegacyId, homeId, dates = {}) {
    const { rows } = await db.query({
      name: 'projects_import_insert',
      text: `
        INSERT INTO projects (customer_id, client_legacy_id, home_id, name, room, room_label, mood_board, space_plan, created_at, updated_at)
  ...
  // Phase 6B: read-only, customer-scoped lookup for the import route's
  // per-record status. INCLUDES soft-deleted rows (see homes-repo.js).
  async function findProjectsByClientLegacyIds(customerId, clientLegacyIds) {
    if (!clientLegacyIds || !clientLegacyIds.length) return [];
    const { rows } = await db.query({
      name: 'projects_find_by_client_legacy_ids',
      text: `SELECT id, client_legacy_id, home_id, deleted_at FROM projects WHERE customer_id = $1 AND client_legacy_id = ANY($2::text[])`,
